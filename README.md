...
- **Consistent world snapshots** on a schedule: the bot runs `save-off` and `save-all flush`, zips the frozen world folder, then turns autosave back on, even when the zip fails halfway
...
- **Retention that fails safe**: keeps `LOCAL_KEEP` zips on disk and `REMOTE_KEEP` of history on Drive, and prunes Drive right after a successful upload. If backups stop, deletion stops with them.
- **Join/leave feed** in the same channel
- **Slash commands**: `/rcon` (full console, owner-locked), `/backup`, `/status`, `/backups`, `/tps`, `/health`, `/profile`
...
| watcher | 60s | uploads new zips to Drive, posts the link, prunes the remote, archives config on change |
| presence | 20s | polls the player list over RCON, posts join/leave diffs |

The bot reads the world folder name from `level-name` in `server.properties` and tracks finished uploads in `state.json`, so a restart re-uploads nothing and skips nothing.
...
| `REMOTE_KEEP` | `12h` | Drive retention, as an rclone duration |
...
- Paper and Spigot keep the nether and end in separate `world_nether` and `world_the_end` folders; add both to `CONFIG_TARGETS`
